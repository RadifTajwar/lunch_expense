        ...
        <template #body="slotProps">
          <div class="flex gap-2">
            <!-- <Button
              ...
              severity="success"
              size="small"
              @click="openAttendances(slotProps.data)"
            /> -->
            <Button
              label="Edit"
              severity="info"
...
const updatedAttendances = ref({}); // { mealId: { userId: { checked, mealCount } } }
...
function fetchData(page = 1) {
  store.dispatch("users/fetchUsers"); // Ensure users are loaded
  store.dispatch("meals/fetchMeals", page);
}

async function saveAttendances({ mealId, updates }) {
  try {
    await store.dispatch("meals/saveAttendances", {
      mealId,
      updates,
    });
...
// Delete meal via store
async function performDelete() {
  console.log("Deleting meal with ID:", paymentToDelete.value);
  try {
